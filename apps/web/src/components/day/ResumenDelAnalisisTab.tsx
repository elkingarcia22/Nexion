  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {data.hallazgos_clave && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {data.hallazgos_clave.decisiones && data.hallazgos_clave.decisiones.length > 0 && (
            <div className="bg-card rounded-3xl border border-white/10 p-6 shadow-xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-sm font-bold text-white uppercase tracking-widest">Decisiones</h4>
              </div>
          ...
          {data.hallazgos_clave.riesgos && data.hallazgos_clave.riesgos.length > 0 && (
            <div className="bg-card rounded-3xl border border-white/10 p-6 shadow-xl">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-red-500/100/10 flex items-center justify-center">
                ...
                <h4 className="text-sm font-bold text-white uppercase tracking-widest">Riesgos</h4>
              </div>
              <ul className="space-y-3">
                {data.hallazgos_clave.riesgos.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-red-500/100/100 mt-2" />
                    <span className="text-sm text-white/70 leading-relaxed">{item}</span>
                  </li>
                ))}
              </ul>
          ...
          {data.hallazgos_clave.oportunidades && data.hallazgos_clave.oportunidades.length > 0 && (
            <div className="bg-card rounded-3xl border border-white/10 p-6 shadow-xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-sm font-bold text-white uppercase tracking-widest">Oportunidades</h4>
              </div>
              <ul className="space-y-3">
                {data.hallazgos_clave.oportunidades.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-green-500/100 mt-2" />
                    <span className="text-sm text-white/70 leading-relaxed">{item}</span>
                  </li>
                ))}
              </ul>
      ...
      {data.proximos_pasos && data.proximos_pasos.length > 0 && (
        <div className="bg-card rounded-3xl border border-white/10 p-8 shadow-xl">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary">
            ...
            <h3 className="text-xl font-bold text-white">Próximos Pasos Sugeridos</h3>
          </div>
          <div className="space-y-3">
            {data.proximos_pasos.map((paso, i) => (
              <div key={paso.id} className="flex items-start gap-4 pb-4 border-b border-white/10 last:border-0">
                <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-card/5 flex items-center justify-center text-white font-bold text-sm">
                  {i + 1}
                </div>
                <div className="flex-1">
                  <p className="text-base text-white/80 font-medium">{paso.texto}</p>
                </div>
                <span className={`flex-shrink-0 text-[10px] font-black px-3 py-1.5 rounded-lg uppercase tracking-widest whitespace-nowrap ${
                  paso.prioridad === "critica"
                    ? "bg-red-500/100/10 text-red-600"
                    : paso.prioridad === "media"
                    ? "bg-amber-500/10 text-amber-600"
                    : "bg-primary/100/10 text-blue-600"
                }`}>
                  {paso.prioridad}
                </span>
              </div>
